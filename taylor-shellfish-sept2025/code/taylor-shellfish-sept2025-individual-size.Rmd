---
title: Taylor Shellfish Sept 2025 resazurin family screening - individual size normalization
author: "AS Huffmyer"
date: '2025'
output:
...
This script analyzes resazurin assays on Taylor Shellfish seed. This script uses individual size normalization.  
...
```{r}
size<-read_xlsx("taylor-shellfish-sept2025/data/size/taylor-shellfish-individual-size.xlsx")%>%select(!notes)
head(size)
```
...
full_data<-left_join(full_data, size, by=c("plate", "well"))
head(full_data)
...
t0_rows <- full_data %>%
  distinct(date, plate, well, family, type, number_oysters, length.mm) %>%   # one per well on each plate
...
Filter to remove any well that did not have 1 single oyster. Only analyze those that we are confident have one oyster in them. 

```{r}
#how many observations before? 
length(full_data$resazurin_counts)

full_data<-full_data%>%
  filter(number_oysters==1)%>%
  select(!number_oysters)

#how many observations after? 
length(full_data$resazurin_counts)

#reduced from 19000+ to 17000+ observations, proceed with this filtering 
```
...
#there are 36 samples that went above the threshold 
...
ggsave(plot1, filename="taylor-shellfish-sept2025/figures/individual_trajectories_individual-normalized.png", width=10, height=8, units="in")
```
...
ggsave(plot2, filename="taylor-shellfish-sept2025/figures/family_trajectories_individual-normalized.png", width=12, height=12, units="in")
...
ggsave(plot2d, filename="taylor-shellfish-sept2025/figures/family_trajectories2_individual-normalized.png", width=8, height=8, units="in")
```
...
ggsave(plot2e, filename="taylor-shellfish-sept2025/figures/family_model_predictions_individual-normalized.png", width=5, height=5, units="in")
...
ggsave(plot2f, filename="taylor-shellfish-sept2025/figures/family_model_predictions_panel_individual-normalized.png", width=12, height=10, units="in")
```
...
ggsave(plot10a, filename="taylor-shellfish-sept2025/figures/auc_family_individual-normalized.png", width=10, height=5, units="in")
```
...
Repeatability (R) value is 0.099 here - most variation is within families with little family structure 
...
There is a significant and moderately positive (r=0.25) correlation. 
...
oyster_compare %>%
  ...
  geom_abline(intercept = 0, slope = 1, linetype = "solid", color = "red", linewidth=1.5) + 
...
Day 2 Total slope 
Day 2 AUC
Day 1 vs 2 Total slope (% diff)
Day 1 vs 2 AUC (% diff)
Ratio day 1:day 2 slope
Ratio day 1:day 2 AUC
Day 1 & Day 2 mean slope
Day 1 & Day 2 mean AUC
Day 1 & Day 2 total AUC
...
metrics <- data.frame(
  ...
  day1_vs_day2_ratio_slope = numeric(0),
  day1_vs_day2_ratio_auc = numeric(0)
)
```
...
Add in AUC data from day 1 
```{r}
data1<-aucs%>%
  group_by(family)%>%
  summarise(day1_auc=mean(AUC))
```

Add in slope data from day 1
```{r}
# nest data 
nested <- full_data %>%
  filter(date=="20250903")%>%
  group_by(unique, family) %>%
  nest()
...
data2 <- nested %>%
  ...
  select(unique, family, slope)%>%
  group_by(family)%>%
  summarise(day1_slope=mean(slope))

metrics<-left_join(data1, data2)
```
...
```{r}
data3 <- full_data %>%
  filter(date=="20250904")%>%
  mutate(time=as.numeric(timepoint))%>%
  group_by(date, plate, well, family) %>%
  summarise(AUC = trapz(time, value))%>%
  group_by(family)%>%
  summarise(day2_auc=mean(AUC))

metrics<-left_join(metrics, data3)
```
...
data4 <- nested %>%
  ...
  select(unique, family, slope)%>%
  group_by(family)%>%
  summarise(day2_slope=mean(slope))

metrics<-left_join(metrics, data4)
```

Add in cumulative mean AUC. 
...
data5 <- full_data %>%
  ...
  summarise(AUC = trapz(time, value))%>%
  group_by(family)%>%
  summarise(cumulative_mean_auc=mean(AUC))

metrics<-left_join(metrics, data5)
```

Add in cumulative mean slopes. 

```{r}
# nest data 
nested <- full_data %>%
  group_by(unique, date, family) %>%
  nest()
...
data6 <- nested %>%
  ...
  select(unique, date, family, slope)%>%
  group_by(family)%>%
  summarise(cumulative_mean_slope=mean(slope))

metrics<-left_join(metrics, data6)

head(metrics)
```

Calculate percent change in slope and auc from day 1 to day 2. 

```{r}
metrics<-metrics%>%
  mutate(day1_vs_day2_perc_change_slope = ((day2_slope-day1_slope)/day2_slope)*100)%>%
  mutate(day1_vs_day2_perc_change_auc = ((day2_auc-day1_auc)/day2_auc)*100)
```

Calculate ratio between day 1 and day 2 slopes and auc (day1:day2). 

```{r}
metrics<-metrics%>%
  mutate(day1_vs_day2_ratio_slope = (day1_slope/day2_slope))%>%
  mutate(day1_vs_day2_ratio_auc = (day1_auc/day2_auc))
```
...
```{r}
metrics%>%
  write_csv("taylor-shellfish-sept2025/output/taylor_screening_metrics_individual-normalized.csv")
```
