...
t0_rows <- full_data %>%
  distinct(date, plate, well, family, type) %>%   # one per well on each plate
...
full_data<-full_data%>%
  rename(value=fluorescence.corr)
  #rename(value=fluorescence.corr.mm)
```

### Remove oversaturated samples

Remove any sample that reached > 19000 raw value at any point during the trials, these were over saturated and data is unreliable. 
...
#there are 39 samples that went above the threshold 
...
Repeatability (R) value is 0.132 here - most variation is within families with little family structure 
...
oyster_data <- full_data %>% filter(unique == "20250903 plate1 A02")%>%
  mutate(time=as.numeric(timepoint))
...
model_results <- full_data %>%
  ...
  unnest(params) %>%
  select(unique, family, term, estimate) %>%
...
## TO DO Clustering metabolic profiles 
